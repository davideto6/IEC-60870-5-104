...
__author__  = "David Abreu"
__version__ = "1.0"
__email__   = "david.abreu@estudiante.uam.es"

'''TCP/IP traffic monitoring system using Scapy and iec104lib.py'''
...
def sniffer():
	sniff(count=0, iface=interfaz, prn = pkt_action, store=0) 
...
def pkt_action(pkt):
	...
	if pkt.haslayer(IP) == 1:
		...
		lenght=Header_Eth+pkt[IP].ihl*4
		...
		if pkt.haslayer(TCP) == 1:
			...
			print bcolors.FAIL+'WARNING NO TCP'	
			...
			if start_value == 104:
				...
				if types==0 and pkt.ApduLen>4: # Type I
					...
					if pkt.haslayer(asdu):
						...
						if (pkt.TypeID<30 and pkt.TypeID>40) or (pkt.TypeID<45 and pkt.TypeID>51) or (pkt.TypeID<58 and pkt.TypeID>64) or (pkt.TypeID<16 and pkt.TypeID%2!=1) or pkt.TypeID==100 or pkt.TypeID==20 or pkt.TypeID==21 or (pkt.TypeID<110 and pkt.TypeID>113) or (pkt.TypeID<108 and pkt.TypeID>100 and pkt.TypeID%2!=1) and pkt[TCP].sport==2404:
							
							if pkt.TypeID==100 or pkt.TypeID==20 or pkt.TypeID==21:
								1+1
							else:
						...
						if (pkt.TypeID >= 45 and pkt.TypeID <= 48) or pkt.TypeID == 100 or pkt.TypeID == 101:
							if pkt.Cause != 7 and pkt.Cause!=10 and pkt[TCP].sport == 2404:
								print 'ATTACK wrong CoT, it must be 7 or 10 instead of ', pkt.Cause, 'in packet number', pkt_counter
								attack+=1

							if pkt.Cause!=6 and pkt[TCP].dport == 2404:
								print 'ATTACK wrong CoT, it must be 6 instead of ', pkt.Cause, 'in packet number', pkt_counter
								attack+=1

							if (pkt.Cause<1 and pkt.Cause>13) or (pkt.Cause<20 and pkt.Cause>41):
								print 'ATTACK wrong CoT range value', pkt.Cause, 'in packet number', pkt_counter
								attack+=1
...
if __name__ == "__main__":
	...
	# bind_layers(TCP, IEC104_s)
	# bind_layers(TCP, IEC104_u)
