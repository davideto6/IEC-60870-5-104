...
def sniffer():
	sniff(offline=captura, prn = pkt_action, store=0)
...
def pkt_action(pkt):
	...
	if pkt.haslayer(IP) == 1:
		...
		if pkt.haslayer(TCP) == 1:
			...
			if pkt[TCP].dport == 2404:
				if number==0:
					maestro[number]=control(pkt[IP].src)
					number+=1
					found_master=0
				
				for i in range(number):
					if (pkt[IP].src==maestro[i].ip):
						found_master=1

				if found_master==0:										
					maestro.append(control(pkt[IP].src))
					number+=1
			else:
				if num==0:
					esclavo[num]=control(pkt[IP].src)
					num+=1
					found_slave=0
				
				for i in range(num):
					if pkt[IP].src==esclavo[i].ip:
						found_slave=1

				if found_slave==0:										
					esclavo.append(control(pkt[IP].src))
					num+=1

		else:
			print bcolors.FAIL+'ATTACK NO TCP'	
				...
				if pkt.ApduLen == 4:
					...
					if types==1: # Type S
						...
						if UType==4:
							...
							u.write(pkt[IP].src)
							u.write("\t")
							u.write(str(pkt[TCP].sport))
							u.write("\t")
							u.write(pkt[IP].dst)
							u.write("\t")
							u.write(str(pkt[TCP].dport))
							u.write("\t")
							u.write("STARTDTact")
							u.write("\n")
				...
				if types==0 and pkt.ApduLen>4: # Type I
					...
					if pkt.haslayer(asdu):
						...
						if pkt.TypeID>127:
							...
							attack+=1
							att.write(pkt[IP].src)
							att.write("\t")
							att.write(pkt[TCP].sport)
							att.write("\t")
							att.write(pkt[IP].dst)
							att.write("\t")
							att.write(pkt[TCP].dport)
							att.write("\t")
							att.write(pkt.TypeID)
							att.write("\t")
							att.write(CoT)
							att.write("\n")
						...
						if (pkt.TypeID<30 and pkt.TypeID>40) or (pkt.TypeID<45 and pkt.TypeID>51) or (pkt.TypeID<58 and pkt.TypeID>64) or (pkt.TypeID<16 and pkt.TypeID%2!=1) or pkt.TypeID==104 or pkt.TypeID==100 or pkt.TypeID==20 or pkt.TypeID==21 or (pkt.TypeID<110 and pkt.TypeID>113) or (pkt.TypeID<108 and pkt.TypeID>100 and pkt.TypeID%2!=1) and pkt[TCP].sport==2404:
							...
							if pkt.TypeID==100 or pkt.TypeID==20 or pkt.TypeID==21 or pkt.TypeID==104:
								...
								att.write(pkt[IP].src)
								att.write("\t")
								att.write(str(pkt[TCP].sport))
								att.write("\t")
								att.write(pkt[IP].dst)
								att.write("\t")
								att.write(str(pkt[TCP].dport))
								att.write("\t")
								att.write(str(pkt.TypeID))
								att.write("\t")
								att.write(str(CoT))
								att.write("\n")
	...
	if pkt_counter == 0:
		...
		if IEC104_counter_i > 0: # print different types iec104-TypeI
			...
			for i in range(numero):
				...
				f.write(lista[i].ip)
				f.write("\t")
				f.write(str(lista[i].tipos))
				f.write("\t")
				f.write(str(lista[i].cont))
				f.write("\n")	
			i=0	
			maestros = "master.csv"
			f = open(maestros,'w')
			for i in range(number):			   
				f.write(maestro[i].ip)
				f.write("\n")
